...
| --- | --- | --- |
| Public Python API | 91-test suite and Docker runner import `skilltrustops.scan` | File and folder entry points |
...
| Security | Per-skill security status, timing and findings | Built-in secrets and dangerous code patterns |
| Privacy | Per-skill privacy status, timing and findings | Email, phone, US SSN and payment card patterns |
...
| Python packaging | Wheel and sdist build successfully | Python 3.12 current release |
...
The current batch scan reads only `SKILL.md`. It does not yet analyze supporting
scripts, references, assets, archives, dependency manifests, install hooks,
semantic prompt injection, encoded/obfuscated instructions, or cross-file data
flows. It does not measure whether a skill improves agent task performance.
